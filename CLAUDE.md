...
- **prompt.ts** — Builds the grader prompt for a single assertion. Includes chain-of-thought instructions and structured JSON output format.
- **grader.ts** — Calls the Claude Agent SDK (`query()`) with Read-only tool access to evaluate one assertion. Extracts JSON verdict from LLM response via `extractVerdict()`.
