...
- **Subscription-first credentials (`--auth`)** — the SDK subprocess prefers `ANTHROPIC_API_KEY` over Claude-subscription OAuth, so `src/auth.ts` shapes the subprocess env to invert that: `auto` (default) withholds `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` when subscription credentials are detected (env token, `~/.claude/.credentials.json`, or macOS Keychain); `subscription` withholds them unconditionally; `api-key` requires `ANTHROPIC_API_KEY` (usage error → exit 1) and withholds `CLAUDE_CODE_OAUTH_TOKEN`. The env is resolved once per run in `runner.ts`/`lint-runner.ts` and shared by all parallel checks via `options.sdkEnv`.
