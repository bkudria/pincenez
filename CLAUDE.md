...
- **Default model: claude-haiku-4-5 for grading** — cheapest/fastest. Per-check `model` field overrides `--model` flag. Lint defaults to claude-sonnet-5 for stronger anti-pattern judgment.
...
- **Unsets `CLAUDECODE` per call via `options.env`** — when pincenez runs inside a Claude Code session, the SDK errors with nested-session failures if `CLAUDECODE=1` is inherited. The env passed to every `query()` call has `CLAUDECODE` unset; the parent's `process.env` is not mutated.
- **Subscription-first credentials (`--auth`)** — the SDK subprocess prefers `ANTHROPIC_API_KEY` over Claude-subscription OAuth, so `src/auth.ts` shapes the subprocess env to invert that: `auto` (default) withholds `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` when subscription credentials are detected (`CLAUDE_SDK_OAUTH_TOKEN`, `~/.claude/.credentials.json`, or macOS Keychain); `subscription` withholds them unconditionally; `api-key` requires `ANTHROPIC_API_KEY` (usage error → exit 1) and withholds both OAuth token variables. An inherited `CLAUDE_CODE_OAUTH_TOKEN` is always stripped (the runtime would let it override a `/login` credential); `CLAUDE_SDK_OAUTH_TOKEN` is the explicit token path, mapped onto `CLAUDE_CODE_OAUTH_TOKEN` in the subprocess env. A run whose only credential is `CLAUDE_CODE_OAUTH_TOKEN` is a usage error. The env is resolved once per run in `runner.ts`/`lint-runner.ts` and shared by all parallel checks via `options.sdkEnv`.
