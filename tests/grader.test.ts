...
describe('gradeCheck', () => {
  ...
  it('returns CheckResult with pass/evidence on success', async () => {
    mockQuery.mockReturnValue(
      asyncMessages([resultMessage('{"pass": true, "evidence": "looks good"}')]),
    );
  ...
  it('defaults cache token counts to 0 when SDK usage omits them', async () => {
    const base = resultMessage('{"pass": true, "evidence": "ok"}');
    mockQuery.mockReturnValue(
      asyncMessages([
        {
  ...
  it('ignores non-result messages in the stream', async () => {
    ...
    mockQuery.mockReturnValue(
      asyncMessages([nonResult, resultMessage('{"pass": true, "evidence": "ok"}')]),
    );
  ...
  it('uses check.model over options.model and default', async () => {
    ...
    await gradeCheck({ ...testCheck, model: 'custom-model' }, '/tmp/out.md', {
      model: 'options-model',
    });
  ...
  it('reports no error details when SDK error has empty errors array', async () => {
    mockQuery.mockReturnValue(asyncMessages([errorMessage('error_unknown', [])]));
  ...
  it('extracts verdict from structured_output when result is empty', async () => {
    ...
    (msg as Record<string, unknown>).structured_output = {
      pass: true,
      evidence: 'found it in structured_output',
    };
    mockQuery.mockReturnValue(asyncMessages([msg]));
  ...
  it('prefers structured_output over result when both present', async () => {
    ...
    (msg as Record<string, unknown>).structured_output = {
      pass: true,
      evidence: 'from structured_output',
    };
    mockQuery.mockReturnValue(asyncMessages([msg]));
  ...
  it('includes raw result snippet when success has unparseable result', async () => {
    mockQuery.mockReturnValue(asyncMessages([resultMessage('this is plain text not json')]));
