...
- **Credentials**: `ANTHROPIC_API_KEY`, `CLAUDE_SDK_OAUTH_TOKEN`, or your Claude Code login is read by the SDK. Pincenez never logs credentials. With `--auth auto` (the default) it withholds the API-key variables from the SDK subprocess when a Claude subscription is detected, so usage is covered by the subscription rather than billed to the key. An inherited `CLAUDE_CODE_OAUTH_TOKEN` is always stripped from the SDK subprocess and never used as a credential — set `CLAUDE_SDK_OAUTH_TOKEN` to provide a token explicitly.
